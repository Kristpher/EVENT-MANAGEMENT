...
const ActiveEvents = ({refetch,searchQueryOrg}) => {
  const navigate = useNavigate();
  const [events,setEvents]=useState(null);
    const BACKEND_URL=process.env.REACT_APP_BACKEND_URL;
  const handleGoFullEvent = (event) => {
    navigate(`/Organizer-active/${encodeURIComponent(event.eventName)}`, { state: {event} });
  };
  useEffect(() => {
  const fetchData = async () => {
    try {
      console.log("organizer approved events");
      const response = await fetch(`${BACKEND_URL}/organizer/active`, {
        method: "GET",
        headers: {
      ...
      if (data.success) {
        setEvents(data.ApprovedEvents);
        console.log("is this undefined",data.ApprovedEvents)
      } else {
